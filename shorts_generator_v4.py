...
미리보기 표시
제작 현황 표시
vrew 자동화: gui 자동화로 tts 자동 생성
자막을 txt로 관리: tts -> srt에서 깨지는 텍스트 보완하는데 활용
제목 저장?
대본 작성 시 몇초 분량인지 실시간 검사?

[로직]
1. 사람이 대본 작성
2. 대본을 vrew에 넣어 tts 생성 (openai api로 가능할 수도 있음)
    1. 강철: 청년 남자(해설 및 주인공)
    2. 성한: 중년 남자
    3. 영미: 중년 여자
    4. 희웅: 청년 남자
    5. 서은: 청년 여자
3. tts를 goldwave로 다듬기 (pydub로 가능할 수도 있음)
4. 다듬은 tts를 vrew에 넣어 srt로 변환 (openai api로 가능할 수도 있음)
5. 이미지 생성 or 서칭
6. 영상 제작
