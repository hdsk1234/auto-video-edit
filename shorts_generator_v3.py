'''
자막 블록을 클래스로 관리한다.
자막 데이터를 json으로 관리한다. -> 이제 껐다 켜도 유지됨. (컨트롤 큐도됨)
'''
...
class SubtitleBlock(tk.Label):
    ...
    def on_backspace_key(self, event):
        ...
        for i in self.manager.SubtitleBlock_list:
            i.row
        # 선택된 빈 블록 제거
        if cur_empty:
...
class SubtitleManager():
    def __init__(self, srt_file_path, json_file_path):
...
class ShortsGeneratorApp:
    def __init__(self, root):
        ...
        # 상단: 프로그램 이름
        self.greeting_label = tk.Label(main_frame, bg=self.bg_color, text="쇼츠 자동 생성기 v3", font=("Helvetica", 24, "bold"))
    ...
    def select_work_dir(self):
        ...
        if folder:
            ...
            self.subtitle_manager = SubtitleManager(srt_file_path, json_file_path)
    ...
    def reset_json(self): # json 파일을 srt파일 상태로 동일하게 초기화한다.
    ...
    def load_files(self):
        paths = {
            "layout_image_file": '/Users/sangwoo-park/ssul-shorts-project/달녹/달녹 레이아웃.png',
